...
class RequestRequest(models.Model):
    ...
    def _get_current_classifier(self):
        ...
        if len(request_classifier_id) > 1:
            raise ValidationError(_(
                'Found more than one Classifier for this Request!'))
    ...
    def _get_tag_ids_domain(self):
        ...
        tag_categ_ids = self.env['generic.tag.category'].browse()

        tag_categ_ids += self.sudo().service_id.tag_category_ids
        tag_categ_ids += self.sudo().category_id.tag_category_ids
        tag_categ_ids += self.sudo().type_id.tag_category_ids
    ...
    # TODO: It seems that this method is not actual for Odoo 16. Check it.
    @api.model
    ...
    def _send_default_notification_created(self, event):
        ...
        if use_special_name and self.email_subject:
            # Replace subject.
    ...
    def name_get(self):
        use_special_name = self.sudo().env['ir.config_parameter'].get_param(
            'generic_request.request_mail_use_special_name')
        if not use_special_name:
            return super().name_get()
        if not self.env.context.get('generic_request_use_special_name'):
            return super().name_get()
        names_data = []
        if self._rec_name in self._fields:
            ...
            for rec in self:
                if rec.email_subject:
                    name = '%(request)s | %(subject)s' % {
                        'request': convert(rec[self._rec_name], rec),
                        'subject': rec.email_subject,
                    }
                    names_data.append((rec.id, name))
                else:
                    names_data.append((
                        rec.id, convert(rec[self._rec_name], rec)))
        else:
            for rec in self:
                if rec.email_subject:
                    name = '%(request)s | %(subject)s' % {
                        'request': '%s,%s' % (rec._name, rec.id),
                        'subject': rec.email_subject,
                    }
                    names_data.append((rec.id, name))
                else:
                    names_data.append((rec.id, '%s,%s' % (rec._name, rec.id)))
        return names_data
